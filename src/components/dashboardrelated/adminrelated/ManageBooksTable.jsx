  ...
  const filteredBooks = books
    ...
    .sort((a, b) => {
      ...
      if (sortField === 'createdAt' || sortField === 'updatedAt') {
        const aVal = a[sortField] ? new Date(a[sortField]).getTime() : 0;
        const bVal = b[sortField] ? new Date(b[sortField]).getTime() : 0;
        // For descending: newer dates come first
  ...
  return (
    <div className="w-full">
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/[0.06] bg-[#0D1033]/40 backdrop-blur-sm">
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-white/6 bg-[#0E1330]/40">
            <span className="text-xs text-[#8890B5]">
              Showing {startIndex + 1}–{Math.min(endIndex, totalItems)} of{' '}
              {totalItems}
            </span>
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => handlePageChange(currentPage - 1)}
                disabled={currentPage === 1}
                className="p-2 rounded-lg border border-white/6 bg-[#0E1330]/50 text-gray-400 hover:text-white hover:bg-[#6D4AFF] disabled:opacity-20 disabled:hover:bg-transparent disabled:hover:text-gray-400 transition-all"
              >
                <ChevronLeft size={16} />
              </button>
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-8 text-xs font-bold rounded-lg border transition-all ${
                      currentPage === page
                        ? 'bg-[#6D4AFF] border-[#6D4AFF] text-white shadow-[0_0_15px_rgba(109,74,255,0.3)]'
                        : 'border-white/6 bg-[#0E1330]/50 text-gray-400 hover:text-white hover:border-white/[0.15]'
                    }`}
                  >
                    {page}
                  </button>
