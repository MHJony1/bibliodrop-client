...
// // 1. api call function for inner component
...
//             Featured{' '}
//             <span className="text-transparent bg-clip-text bg-linear-to-r from-[#FFF] via-[#FFEBB3] to-[#F7B500]">
//               Masterpieces
//             </span>
//           </h2>
...
import { getBooks } from '@/lib/api/books';
import { Sparkles, Library, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
import BookCard from '@/components/booksrelated/BookCard';
import SkeletonLoader from '@/components/SkeletonLoader';
import Link from 'next/link';
  ...
  if (isMobile) {
    return (
      <div className="relative w-full">
        ...
        >
          {books.map((book) => (
            <SwiperSlide key={book?._id?.['$oid'] || book?._id || Math.random()}>
              <div className="hover:-translate-y-2 transition-all duration-300 ease-out">
                <BookCard book={book} />
              </div>
            </SwiperSlide>
          ))}
    ...
    <section className="w-full py-24 relative bg-[#060814] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
          ...
          <p className="text-xs sm:text-sm text-gray-400 font-medium m-0 leading-relaxed mt-3">
            Explore our curated collection of{' '}
            <span className="text-[#F7B500] font-bold">8 premium masterpieces</span>{' '}
            from elite worldwide repositories.
          </p>
        </div>
