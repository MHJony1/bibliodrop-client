...
import { ArrowLeft, Calendar, DollarSign, User, CreditCard, Bookmark } from 'lucide-react';
...
export default async function BookDetailsPage({ params }) {
  ...
  if (!book) {
    return (
      <div className="min-h-screen bg-[#0D1530] flex items-center justify-center text-white">
        <p className="text-xl font-medium text-gray-400">Book data loading failed or not found!</p>
      </div>
    );
  }
  ...
  const isLibrarianOwner = currentUser?.role === 'librarian' && bookOwnerId === currentUserId;
  const hasPurchased = book?.buyers?.includes(currentUserId) || false; 

  // 2. price calculation
  const bookPrice = book?.price !== undefined ? Number(book.price) : 0;
  const deliveryFee = book?.deliveryFee !== undefined ? Number(book.deliveryFee) : 0;
  const totalPayable = bookPrice + deliveryFee;

  // 3. categoru color theme
  ...
  const catStyle = categoryColors[currentCategory] || 'bg-teal-500/15 text-teal-200 border-teal-500/30';

  const formattedDate = book?.dateAdded 
    ? new Date(book.dateAdded).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
    : 'N/A';
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-[#121B3A] via-[#0E152E] to-[#0A0E22] text-[#D1D5DB] antialiased">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-32 pb-8">
        ...
        {/* Back navigation */}
        <div className="mb-8">
          <Link href="/browsebooks" className="inline-flex items-center gap-2 text-sm text-[#A0AEC0] hover:text-white transition-colors group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 items-center">
          ...
          <div className="w-full flex justify-center md:justify-start">
            <div className="relative w-full max-w-107.5 md:max-w-112.5 aspect-square rounded-[2rem] overflow-hidden bg-[#16224F]/40 border border-white/8 shadow-[0_20px_50px_rgba(0,0,0,0.4)] transition-all duration-300 hover:border-white/15">
              <Image
                src={book?.coverImage || 'https://images.unsplash.com/photo-1544947950-fa07a98d237f?w=600'}
              ...
              <div className="absolute top-4 right-4 z-10">
                <span className="px-3 py-1 rounded-full text-xs font-bold tracking-wide uppercase border backdrop-blur-md bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
          ...
          <div className="flex flex-col gap-5 justify-center">
            <div className="space-y-2.5">
              <span className={`inline-block px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider border ${catStyle}`}>
              ...
              <p className="text-sm sm:text-base text-[#A3AED0]">
                by <span className="text-white font-semibold">{book?.author || 'Unknown Author'}</span>
              </p>
            </div>
            ...
            <div className="bg-[#111A3E]/80 border border-white/[0.06] backdrop-blur-sm rounded-2xl p-5 max-w-md space-y-3.5 shadow-xl">
              <div className="flex items-center justify-between border-b border-white/[0.04] pb-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <p className="text-xs sm:text-sm text-[#A3AED0]">Book Price</p>
                </div>
                <p className="text-white text-sm font-bold">${bookPrice.toFixed(2)}</p>
              </div>

              <div className="flex items-center justify-between border-b border-white/[0.04] pb-2">
                <div className="flex items-center gap-2.5">
                  <DollarSign size={16} className="text-[#A3AED0]" />
                  <p className="text-xs sm:text-sm text-[#A3AED0]">Delivery Fee</p>
                </div>
                <p className="text-[#F5C842] text-sm font-bold">${deliveryFee.toFixed(2)}</p>
              </div>

              {/* স্ট্রাইপ গেটওয়ের টোটাল এমাউন্ট */}
              <div className="flex items-center justify-between bg-white/[0.02] -mx-5 -mb-3 p-3 px-5 border-t border-white/[0.05] rounded-b-2xl">
                <div className="flex items-center gap-2.5">
                  ...
                  <p className="text-xs uppercase tracking-wider font-bold text-indigo-300">Total Payable</p>
                </div>
                <p className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400 text-base font-black">${totalPayable.toFixed(2)}</p>
              </div>
            </div>

            {/* 💎 হাইলাইটেড মেটা ইনফো প্যানেল (Librarian ও Date সেকশন বড় ও প্রিমিয়াম করা হয়েছে) */}
            <div className="flex flex-wrap gap-3 max-w-md">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-xs font-medium text-[#B0BBD8]">
                ...
                <span>Listed by: <strong className="text-white font-semibold">Librarian</strong></span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-xs font-medium text-[#B0BBD8]">
                <Calendar size={14} className="text-purple-400" />
                <span>Date Cataloged: <strong className="text-white font-semibold">{formattedDate}</strong></span>
              </div>
            </div>
      ...
      {/* Reviews Block  */}
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-white/[0.06] pt-12 pb-20">
        <ReviewsSection 
          bookId={bookId} 
          currentUser={currentUser} 
