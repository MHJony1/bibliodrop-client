...
export default async function BrowseBooksPage({ searchParams }) {
  ...
  if (response?.success && Array.isArray(response.data)) {
    books = response.data; pagination = response.pagination || null;
  } else if (response?.data && Array.isArray(response.data)) {
    books = response.data; pagination = response.pagination || null;
  } else if (Array.isArray(response)) {
    books = response;
  } else if (response?.books && Array.isArray(response.books)) {
    books = response.books; pagination = response.pagination || null;
  }
      ...
      <div className="max-w-[1200px] mx-auto relative z-10">
        <div className="mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white leading-tight mb-3">
            Browse{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#A78BFA] to-[#6C47FF]">Books</span>
          </h1>
          {totalBooks > 0 ? (
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-white/[0.04] border border-white/[0.06]">
                <span className="text-[11px] text-[#8890B5]">Total in library:</span>
                <span className="text-[11px] font-bold text-white">{totalBooks} books</span>
              </div>
              ...
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[#6C47FF]/10 border border-[#6C47FF]/20">
                ...
                <span className="text-[11px] font-bold text-white">{showingFrom}–{showingTo} of {totalBooks}</span>
                <span className="text-[11px] text-[#8890B5]">(page {currentPage}/{totalPages})</span>
              </div>
            </div>
          ) : (
            <p className="text-sm text-[#8890B5]">No books found matching your filters.</p>
          )}
        </div>
        ...
        {books.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 rounded-2xl border border-dashed border-white/[0.07] bg-[#0D1033]/40 text-center">
            ...
            <p className="text-[#8890B5] text-sm font-medium mb-1">No books match your filters.</p>
            <p className="text-[#565C7A] text-xs">Try adjusting your search or clearing filters.</p>
          </div>
        ) : (
          <>
